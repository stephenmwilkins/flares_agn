...
for i, tag in enumerate(np.flip(fl.tags)):
    ...
    N_weighted_intrinsic, edges_intrinsic = np.histogram(y_intrinsic, bins=bins, weights=ws)
    ...
    axes.flatten()[i].plot(bins[:-1][Ns_agn] + binw / 2, np.log10(phi[Ns_agn]), ls='dotted', c=cmap(norm(z)))
    axes.flatten()[i].plot(bins[:-1][Ns_intrinsic] + binw / 2, np.log10(phi_intrinsic[Ns_intrinsic]), ls='dashed', c=cmap(norm(z)))
...
fig.savefig(f'figures/uvlf_intrinsic_and_dust_grid.pdf', bbox_inches='tight')
fig.clf()
