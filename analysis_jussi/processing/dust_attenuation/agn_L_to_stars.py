...
for i, tag in enumerate(np.flip(fl.tags)):
    ...
    y_int = np.log10(10**l_agn_int / 10 ** lstar[s_t])
    ...
    out_int = flares.binned_weighted_quantile(x, y_int, ws, bins, quantiles)
    ...
    axes.flatten()[i].plot(bincen[Ns], out[:, 1][Ns], ls='--', c=cmap(norm(z)))
    axes.flatten()[i].fill_between(bincen[Ns], out[:, 0][Ns], out[:, 2][Ns], color=cmap(norm(z)),
                                   alpha=0.4)

    axes.flatten()[i].plot(bincen, out_int[:, 1], c=cmap(norm(z)), ls=':')
    axes.flatten()[i].plot(bincen[Ns], out_int[:, 1][Ns], c=cmap(norm(z)))
    axes.flatten()[i].fill_between(bincen[Ns], out_int[:, 0][Ns], out_int[:, 2][Ns], color=cmap(norm(z)),
                                   alpha=0.4)

    axes.flatten()[i].axhline(0, alpha=0.8, c='k', ls='--', linewidth=1)

    s_outshine = (y > 0)
    s_outshine_int = (y_int > 0)

    print(z, np.sum(s_outshine), np.sum(s_outshine_int))

    axes.flatten()[i].scatter(mstar[s_t][s_outshine_int], y_int[s_outshine_int], s=5, color=cmap(norm(z)), alpha=0.8)
    axes.flatten()[i].scatter(mstar[s_t][s_outshine], y[s_outshine], s=5, marker="^", color='r', alpha=1)
...
axes.flatten()[-1].scatter(-99, -99, color='k', s=7, alpha=0.8, label=fr"Outshine, intrinsic", zorder=0)
axes.flatten()[-1].scatter(-99, -99, color='r', s=7, marker="^", alpha=0.8, label=fr"Outshine, attenuated", zorder=1)
axes.flatten()[-1].plot(-99, -99, color='k', ls='-', alpha=0.8, label=fr"AGN, intrinsic", zorder=2)
axes.flatten()[-1].plot(-99, -99, color='k', ls='--', alpha=0.8, label=fr"AGN, attenuated", zorder=3)
axes.flatten()[-1].legend(loc="lower center", prop={'size': 7})
...
fig.savefig(f'figures/agn_fuv_dust_frac_grid_mstar_coarse_int+dust.pdf', bbox_inches='tight')
fig.clf()