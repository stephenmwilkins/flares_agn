...
ion_bolcorr = interp1d(bol_correction['AGN_T'], bol_correction['ratio']['ionising'])
...
xi_gal = pickle.load(open(flares_dir+'/ion_luminosity.p', 'rb'))
...
for i, tag in enumerate(np.flip(fl.tags)):
    ...
    ion_agn = (ion_bolcorr(b[s_t]))*10**q

    x = mstar[s_t]

    xi = np.log10(xi[s_t])

    print(f'Ionising photon luminosities at z = {z}')
    print('AGN')
    print(f'min = {min(np.log10(ion_agn)):.2f}, mean = {np.mean(np.log10(ion_agn)):.2f}, max = {max(np.log10(ion_agn)):.2f}')
    print('stars')
    print(f'min = {min(xi):.2f}, mean = {np.mean(xi):.2f}, max = {max(xi):.2f}')

    y = np.log10(ion_agn)-xi
...
fig.text(0.01, 0.55, r'$\rm log_{10}[L_{ion, AGN} \; / \; L_{ion, Stellar}]$', ha = 'left', va = 'center', rotation = 'vertical', fontsize=10)
fig.text(0.45,0.05, r'$\rm log_{10}[M_{*}\;/\;M_{\odot}]$', ha = 'center', va = 'bottom', fontsize=10)

fig.savefig(f'figures/ion_luminosity_ratio_grid.pdf', bbox_inches='tight')
fig.clf()
