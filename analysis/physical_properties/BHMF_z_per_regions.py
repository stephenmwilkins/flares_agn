...
for ii in range(len(halo)):
    ...
    for i, tag in enumerate(fl.tags):
        ...
        ws[tag] = np.append(ws[tag], np.ones(np.shape(X[halo[ii]][tag][s]))*weights[ii])
        x[tag] = np.append(x[tag], np.log10(X[halo[ii]][tag][s]))


        x[tag] += 10 # units are 1E10 M_sol

        N_weighted, edges = np.histogram(x[tag], bins = bins)#, weights = ws[tag])

        phi = N_weighted/(binw*vol)

        ax.plot(bins[:-1] + binw / 2, np.log10(phi), c=cmap(norm(z)), label = rf'$\rm z={int(z)}$')

    ax.text(0.1, 0.9, f'region = {ii}', fontsize=8, transform=ax.transAxes,
                           color='k', ha='left')
    ...
    fig.savefig(f'figures/z_per_regions/MBH_DF_{ii}.pdf', bbox_inches='tight')
    fig.clf()
