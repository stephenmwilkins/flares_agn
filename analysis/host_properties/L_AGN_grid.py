...
for i, tag in enumerate(np.flip(fl.tags)):
    ...
    bins = np.arange(7, 12, 0.25) # x-coordinate bins, in this case stellar mass
...
fig.text(0.01, 0.55, r'$\rm log_{10}[L_{AGN, bolometric} \; / \; erg\;s^{-1}]$', ha = 'left', va = 'center', rotation = 'vertical', fontsize=10)
fig.text(0.45,0.05, r'$\rm log_{10}[M_{*}\;/\;M_{\odot}]$', ha = 'center', va = 'bottom', fontsize=10)
fig.savefig(f'figures/L_AGN_grid.pdf', bbox_inches='tight')
fig.clf()
