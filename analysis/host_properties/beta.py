...
for j, phot_type in enumerate(phot_types):
    ...
    for i, tag in enumerate(np.flip(fl.tags)):
        ...
        for f in filters:
            l[f] = np.array([])
            L[f] = fl.load_dataset(f,
                                        arr_type=f'Galaxy/BPASS_2.2.1/Chabrier300/Luminosity/{phot_type}/')  # ergs/s/Hz
        ...
        for ii in range(len(halo)):
            ...
            for f in filters:
                l[f] = np.append(l[f], L[f][halo[ii]][tag][s])
        ...
        y_nonhost = beta(l_fuv, l_nuv, l['FUV'][w_nonhost], l['NUV'][w_nonhost])

        y = beta(l_fuv, l_nuv, l['FUV'][w], l['NUV'][w])
        ...
        axes.flatten()[i].text(0.8, 0.9, r'$\rm z={0:.0f}$'.format(z), fontsize=8, transform=axes.flatten()[i].transAxes,
                               color=cmap(norm(z)))
        axes.flatten()[i].set_ylim(-2.9,-1)
        axes.flatten()[i].set_xlim(7, 11)

        axes.flatten()[i].set_yticks([-2.5, -2, -1.5, -1])
