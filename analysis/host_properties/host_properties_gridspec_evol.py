...
def plot_cell(ax, tag_index, x_name, y_name):
    ...
    ax.plot(bincen, out[:, 1], c=cmap(norm(c)), ls='-')
    #ax.fill_between(bincen[Ns], out[:,0][Ns], out[:,2][Ns], color=cmap(norm(c)), alpha = 0.15)
...
for i, tag in enumerate(tags):
    plot_cell(ax101, i, properties[0], properties[1])
...
for i, tag in enumerate(tags):
    c = zeds[i]
    ax100.plot(-99, -99, c=cmap(norm(c)), ls='-', label=rf"$\rm z = {c:.0f}$")
    ax100.set_xlim(limits[properties[0]])
    ax100.set_ylim(limits[properties[1]])
ax100.legend(loc="center")

# top left
ax111 = plt.subplot(gs1[2])
for i, tag in enumerate(tags):
    plot_cell(ax111, i, properties[0], properties[-2])
...
for i, tag in enumerate(tags):
    plot_cell(ax122, i, properties[1], properties[-1])
...
fig.savefig(f'figures/host_gridspec_evol.pdf', bbox_inches='tight')
