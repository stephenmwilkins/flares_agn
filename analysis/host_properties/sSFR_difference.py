...
for i, tag in enumerate(np.flip(fl.tags)):
    ...
    #axes.flatten()[i].fill_between(bincen[Ns], out[:, 0][Ns], out[:, 2][Ns]/10**6, color=cmap(norm(z)), alpha=0.2)
    ...
    axes.flatten()[i].text(0.7, 0.9, r'$\rm z={0:.0f}$'.format(z), fontsize=8, transform=axes.flatten()[i].transAxes,
                           color=cmap(norm(z)))
    axes.flatten()[i].set_ylim(9.8,12)
    axes.flatten()[i].set_xlim(7, 11)
    axes.flatten()[i].set_yticks([10, 11, 12])
