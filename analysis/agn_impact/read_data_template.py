...
for i, run in enumerate(runs):
    ...
    for ii, tag in enumerate(tags):
        ...
        axes.flatten()[ii].set_ylim(-3, 3)
        axes.flatten()[ii].set_xlim(7.5, 11.2)

        axes.flatten()[ii].text(0.1, 0.9, r'$\rm z={0:.0f}$'.format(zed), fontsize=8,
                               transform=axes.flatten()[ii].transAxes,
                               color=cmap(norm(zed)))

    fig.text(0.01, 0.55, r'$\rm log_{10}[SFR_{inst} \; / \; M_{\odot}\;yr^{-1}]$', ha='left', va='center',
             rotation='vertical', fontsize=10)
    fig.text(0.45, 0.05, r'$\rm log_{10}[M_{*}\;/\;M_{\odot}]$', ha='center', va='bottom', fontsize=10)

    fig.savefig(f'figures/SFR_Mstar_{run_labels[i]}.pdf', bbox_inches='tight')
    fig.clf()
