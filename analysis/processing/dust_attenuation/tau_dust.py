...
BHLOS = pickle.load(open(f'{flares_dir}/bh_los.p', 'rb'))
...
fig.subplots_adjust(left=0.07, bottom=0.15, top=1, right=0.85, wspace=0.0, hspace=0.0)
for i, tag in enumerate(np.flip(fl.tags)):
    ...
    axes.flatten()[i].set_ylim(-0.1, 55)
...
fig.text(0.01, 0.55, r'$\rm \tau_{1500\AA,\,dust}$', ha='left', va='center',
         rotation='vertical', fontsize=10)
fig.text(0.45, 0.05, r'$\rm log_{10}[M_{*}\;/\;M_{\odot}]$', ha='center', va='bottom',
         fontsize=10)

fig.savefig(f'figures/agn_tau_dust_grid_not_logged_newfile.pdf', bbox_inches='tight')
fig.clf()