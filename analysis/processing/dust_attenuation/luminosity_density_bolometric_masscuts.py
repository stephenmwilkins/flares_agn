...
for j, mass_cut in enumerate(masslimits):
    label = f"Mass>{mass_cut}"
    ax1.plot(np.flip(fl.zeds), np.log10(density["gal"]["bolometric"][label]), '-', color=cmap_mass(norm_mass(mass_cut)), alpha=0.8)

    ax1.plot(np.flip(fl.zeds), np.log10(density["agn"]["bolometric"][label]), ':', color=cmap_mass(norm_mass(mass_cut)), alpha=0.8)

ax1.plot([-99, -99], [-99, -99], 'k-', alpha=0.7, label="Stellar")
ax1.plot([-99, -99], [-99, -99], 'k:', alpha=0.7, label="AGN")
...
ax1.set_ylabel(r"$\rm log_{10}[\rho_{bol} \, / \, erg \, s^{-1} \, Mpc^{-3}]$")

plt.savefig('figures/lum_density_bolometric_masscuts.pdf', bbox_inches='tight')
