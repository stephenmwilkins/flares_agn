...
for i, tag in enumerate(np.flip(fl.tags)):
    ...
    axes.flatten()[i].scatter(mstar+10, y, color='k', alpha=0.5, s=4, zorder=5)
    s1 = (x>=45)
    axes.flatten()[i].scatter(mstar[s1]+10, y[s1], c=cmap(norm(z)), alpha=0.7, s=4, zorder=10)
    ...
    axes.flatten()[i].set_xlim(9.5,12.)
