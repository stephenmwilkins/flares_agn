...
for i, tag in enumerate(np.flip(fl.tags)):
    ...
    ax.scatter(mhalo, lagn, c=cmap(norm(dt)), alpha=0.7, s=4, zorder=-99)
    ...
    ax.set_xlim(9.9,13.5)
    ax.set_ylim(43.5, 47.1)
    ...
    ax.set_xlabel(r'$\rm log_{10}[M_{subhalo}\;/\;M_{\odot}]$')
    ax.set_ylabel(r'$\rm log_{10}[L_{AGN}\;/\;erg\,s^{-1}]$')

    fig.savefig(f'figures/LAGN_Mhalo/LAGN_Mhalo_{z}.pdf', bbox_inches='tight')
    fig.clf()
