...
for i, tag in enumerate(np.flip(fl.tags)):
    ...
    for ii in range(len(halo)):
        ...
        x = np.log10(Y[halo[ii]][tag][s])+10
        y = np.log10(Mdot[halo[ii]][tag][s])
        delta = deltas[ii]

        b = np.array([l_agn(q, etta_r=0) for q in y])
        lagn = b
        bins = np.arange(42, 48, 1)  #  x-coordinate bins, in this case stellar mass
        bincen = (bins[:-1] + bins[1:]) / 2.

        N_flares, binedges = np.histogram(lagn, bins=bins)
        ax.plot(bincen, np.log10(N_flares), c=cmap(norm(delta)), alpha=1, zorder=-99)
    ...
    ax.text(0.8, 0.9, r'$\rm z={0:.0f}$'.format(z), fontsize=8, transform=ax.transAxes, color='k')

    ax.set_yticks([0, 1])
    ax.set_ylim(0, 1.6)
    ax.set_xticks([44, 45,46,47])
    ax.set_xlim(42.9, 47.1)
    ...
    ax.set_xlabel(r'$\rm log_{10}[L_{AGN}\;/\;erg\,s^{-1}]$')
    ax.set_ylabel(r'$\rm log_{10}[N_{AGN}]$')

    fig.savefig(f'figures/LAGN_hist/LAGN_{z}_hist.pdf', bbox_inches='tight')
    fig.clf()
