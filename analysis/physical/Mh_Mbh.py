...
xlimits = np.array([11.1, 13.9])
ylimits = np.array([5.1, 9.49])
...
# quantities.append({'path': 'Galaxy', 'dataset': f'M500',
#                   'name': 'M500', 'log10': True})
quantities.append({'path': 'Galaxy', 'dataset': f'SubhaloMass',
                  'name': 'M500', 'log10': True})
...
ax.scatter(D['log10M500'][s], D['log10BH_Mass'][s], s=1, c=cmap(norm(log10LbolLedd[s])))

ax.set_ylim(ylimits)
ax.set_xlim(xlimits)

ax.set_xlabel(r'$\rm log_{10}(M_{500}/M_{\odot})$')
