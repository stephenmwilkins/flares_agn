...
for z, c, ax in zip(redshifts, cmr.take_cmap_colors('cmr.bubblegum_r', len(redshifts)), axes.flatten()):
    ...
    log10bol = np.arange(45.001, 48, 0.01)
    R = 1E45/(10**log10bol-1E45)
    ax.fill_between(log10bol, -10*np.ones(len(log10bol)), np.log10(R),color='k',alpha=0.05)
    ...
    # get bolometric luminosity
    blackhole_accretion_rates = D['BH_Mdot'] * u.accretion_rate_units # inludes h
    blackhole_bolometric_luminosity = u.calcualte_bolometric_luminosity(blackhole_accretion_rates)
    log10Lbhs = np.log10(blackhole_bolometric_luminosity.to('erg/s'))
...
fig.savefig(f'figs/relative_Lbol.pdf')
fig.savefig(f'figs/relative_Lbol.png')
